...
//  For Stand Text popup
  ...
  function SetStandardTxtValue(BaseId,CtrlID,MaxCount,Standardtxt) {debugger
   if(Standardtxt!=''){
   var PrevVal = ''
...
{ 
   ...
   // if (FunctId !=0) QAMS_SpecialValidations(FunctId,BaseID,CtrlId,document.getElementById("IssueID").value)
    ...
    function WorkAllUserGrpPopup(BaseId,CtrlID,IsuLogid,IsuTypeId,CusValFunId)
     ...
     var InitideptName='0'
        ...
        var InintdeptcrlId = 'FLD_183_549'
        var IHodId = ''
        ...
        if (BaseId=='FLD_219')  // Deviation
        {  
          IHodId = 'FLD_216' + '_' + CtrlID
        ...
        if (BaseId=='FLD_238')  // Permanent And Temporary Change Control Process
        {  
          IHodId = 'FLD_237' + '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
        ...
        if (BaseId=='FLD_94')  // CAPA
        {
        
          IHodId= 'FLD_93'+ '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
 ...
 if (BaseId=='FLD_244')  // CCT
        {
        
          IHodId= 'FLD_243'+ '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
       ...
       // For Handling Additional Departments Selected not to come in Other Departments During Return
        ...
        if (BaseId=='FLD_93')  // CAPA
        {
        
          IHodId= 'FLD_94'+ '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
